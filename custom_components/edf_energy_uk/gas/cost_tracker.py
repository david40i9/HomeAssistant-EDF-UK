...
class _GasCostTrackerBase(CoordinatorEntity, EDFEnergyGasSensor, RestoreSensor):
    """Base for gas daily/weekly/monthly cost sensors."""
...
class EDFEnergyGasWeeklyCost(_GasCostTrackerBase):
    ...
    @property
    def unique_id(self):
        return f'edf_energy_gas_{self._serial_number}_{self._mprn}_weekly_cost'

    @property
    def name(self):
        return f'EDF Gas Weekly Cost ({self._serial_number}/{self._mprn})'

    def _extract(self, result):
        return result.weekly_consumption, result.weekly_cost, result.weekly_reset


class EDFEnergyGasMonthlyCost(_GasCostTrackerBase):
    """Gas cost so far this month (1st–now)."""

    @property
    def unique_id(self):
        return f'edf_energy_gas_{self._serial_number}_{self._mprn}_monthly_cost'

    @property
    def name(self):
        return f'EDF Gas Monthly Cost ({self._serial_number}/{self._mprn})'

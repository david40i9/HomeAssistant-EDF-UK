...
async def async_refresh_gas_standing_charges_data(
    ...
    existing_result: GasStandingChargeCoordinatorResult | None,
):
    ...
    if existing_result is None or current >= existing_result.next_refresh:
        ...
        if (existing_result is not None and
            ...
            _LOGGER.info('Cached gas standing charge still valid, reusing')
            ...
            try:
                ...
                _LOGGER.debug(f'Gas standing charge retrieved for {target_mprn}/{target_serial_number} ({tariff.code})')
            except Exception as e:
                if not isinstance(e, ApiException):
                    raise
                raised_exception = e
                _LOGGER.debug(f'Failed to retrieve gas standing charge for {target_mprn}/{target_serial_number} ({tariff.code})')
        ...
        if existing_result is not None:
            result = GasStandingChargeCoordinatorResult(
            ...
            if result.request_attempts == 2:
                _LOGGER.warning(f"Failed to retrieve gas standing charge for {target_mprn}/{target_serial_number} - using cached.")
        else:
            result = GasStandingChargeCoordinatorResult(
            ...
            _LOGGER.warning(f"Failed to retrieve gas standing charge for {target_mprn}/{target_serial_number}.")
...
async def async_setup_gas_standing_charges_coordinator(hass, account_id: str, target_mprn: str, target_serial_number: str):
    ...
    async def async_update_data():
        ...
        hass.data[DOMAIN][account_id][key] = await async_refresh_gas_standing_charges_data(
            current, client, account_info, target_mprn, target_serial_number, existing
        )

...
class EDFEnergyElectricityCurrentStandingCharge(CoordinatorEntity, EDFEnergyElectricitySensor, RestoreSensor):
    """Sensor for displaying the current electricity standing charge."""
    ...
    @property
    def name(self):
        return f'EDF {self._export_name_addition}Electricity Standing Charge ({self._serial_number}/{self._mpan})'
    ...
    def _handle_coordinator_update(self) -> None:
        result: ElectricityStandingChargeCoordinatorResult = self.coordinator.data if self.coordinator is not None and self.coordinator.data is not None else None

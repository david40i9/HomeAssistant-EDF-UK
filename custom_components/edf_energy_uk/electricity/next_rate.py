...
class EDFEnergyElectricityNextRate(CoordinatorEntity, EDFEnergyElectricitySensor, RestoreSensor):
    """Sensor for displaying the next electricity rate."""
    ...
    @property
    def unique_id(self):
        return f'edf_energy_electricity_{self._serial_number}_{self._mpan}{self._export_id_addition}_next_rate'

    @property
    def name(self):
        return f'EDF {self._export_name_addition}Electricity Next Rate ({self._serial_number}/{self._mpan})'
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if rates_result is not None:
            ...
            if rate_information is not None:
                ...
                self._attributes["start"] = rate_information["next_rate"]["start"]
                self._attributes["end"] = rate_information["next_rate"]["end"]
            else:
                self._state = None
    ...
    async def async_added_to_hass(self):
        ...
        if state is not None and last_sensor_state is not None and self._state is None:
            ...
            _LOGGER.debug(f'Restored EDFEnergyElectricityNextRate state: {self._state}')
